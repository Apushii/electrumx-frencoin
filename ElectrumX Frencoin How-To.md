...
- Download latest version of electrumx frencoin with **git clone https://github.com/Apushii/electrumx.git** or download from the link above (e.g. wget <path>) – the tar.gz version is suggested as that can easily be unpacked with tar –xzf {name}.tar.gz . 
